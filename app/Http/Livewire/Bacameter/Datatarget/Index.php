...
class Index extends Component
{
    ...
    {
        $data = BacaMeter::with('pengguna')->when($this->statusBaca == 0, fn($q) => $q->whereNull('tanggal_baca'))->when($this->unitPelayanan, fn($q) => $q->whereIn('jalan_id', Regional::where('unit_pelayanan_id', $this->unitPelayanan)->get()->pluck('id')))->when($this->statusBaca == 1, fn($q) => $q->whereNotNull('tanggal_baca'))->where('periode', $this->tahun . '-' . $this->bulan . '-01')->where(fn($q) => $q->orWhereHas('pelanggan', fn($q) => $q->where('nama', 'like', '%' . $this->cari . '%')->where('no_langganan', 'like', '%' . $this->cari . '%')))->when($this->tanggalBaca, fn($q) => $q->where(DB::raw('date(tanggal_baca)'), date('Y-m-d', strtotime($this->tanggalBaca))));
