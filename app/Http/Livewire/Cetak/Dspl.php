...
class Dspl extends Component
{
    ...
    public function render()
    {
        return view('livewire.cetak.dspl', [
            'data' => ModelsDspl::with('golongan')->with('pelanggan')->where('periode', $this->tahun . '-' . $this->bulan . '-01')->when($this->golongan, fn ($q) => $q->where('golongan_id', $this->golongan))->when($this->unitPelayanan, fn ($q) => $q->whereIn('rayon_id', Regional::where('unit_pelayanan_id', $this->unitPelayanan)->get()->pluck('id')))->when($this->rayon, fn ($q) => $q->where('rayon_id', $this->rayon))->orderBy('pelanggan_id')->orderBy('periode_rekening')->paginate(10)
        ]);
    }
}
