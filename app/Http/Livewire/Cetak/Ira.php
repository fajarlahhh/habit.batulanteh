...
class Ira extends Component
{
    ...
    public function cetak()
    {
        $cetak = view('cetak.ira', [
            ...
            ifnull(sum(biaya_materai),0) materai')->where('periode', $this->tahun . '-' . $this->bulan . '-01')->when($this->unitPelayanan, fn ($q) => $q->whereIn('rayon_id', Regional::where('unit_pelayanan_id', $this->unitPelayanan)->get()->pluck('id')))->when($this->rayon, fn ($q) => $q->where('rayon_id', $this->rayon))->leftJoin('ira', 'golongan.id', '=', 'ira.golongan_id')->groupBy('golongan.id')->get(),
        ])->render();
