...
class Progresbacameter extends Component
{
    ...
    public function cetak()
    {
        $cetak = view('cetak.progresbacameter', [
            ...
            'data' => Pengguna::whereIn('id', BacaMeter::where('periode', $this->tahun . '-' . $this->bulan . '-01')->get()->pluck('pembaca_id'))->when($this->unitPelayanan, fn ($q) => $q->whereHas('bacaMeter', fn ($r) => $r->whereIn('jalan_kelurahan_id', Regional::where('unit_pelayanan_id', $this->unitPelayanan)->get()->pluck('id'))))->when($this->rayon, fn ($q) => $q->where('rayon_id', $this->rayon))->with(['bacaMeter' => function ($q) {
