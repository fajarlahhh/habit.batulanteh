...
class Index extends Component
{
    ...
    public function render()
    {
        return view('livewire.pengaturan.pengguna.index', [
            'data' => Pengguna::where('uid', '!=', 'admin')->with('unitPelayanan')->when($this->exist == '2', fn($q) => $q->onlyTrashed())->where(fn($q) => $q->where('uid', 'like', '%' . $this->cari . '%')->orWhere('nama', 'like', '%' . $this->cari . '%'))->orderBy('uid')->paginate(10),
            'no' => ($this->page - 1) * 10,
        ]);
    }
}
